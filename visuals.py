...
def viewsinglecf(dates, loan, prop, rent, oexpenses, interest, amort, debt, totflow):
    ...
    rowlabels = ['Dates', 'Starting debt', 'Loan', 'Property buy/sell', 'Rent', 'Operating expenses', 'Interest', 'Amortization', 'Final debt', 'Total cash flow']
    df = pd.DataFrame(row, index = rowlabels)
    df.columns.name = 'Month #'
    return df

def viewportcf(names, alldates, amort, allcf, debt, totcf):
    ...
    debt = convtomoney(debt)
    row = [alldates.date, debt, convtomoney(amort)]
    for arr in allcf:
        row.append(convtomoney(arr)) # convert floats to money objects
    row.append(np.roll(debt, -1))
    row.append(convtomoney(totcf))
    rowlabels = ['Dates', 'Starting debt', 'Amortization'] + names + ['Final debt', 'Total cash flow']
    df = pd.DataFrame(row, index = rowlabels)
    df.columns.name = 'Month #'
    return df
